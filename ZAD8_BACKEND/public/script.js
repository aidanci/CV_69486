...
function renderExpenses() {
    ...
    if (expenses.length === 0) {
        expensesList.innerHTML = "<p>Brak zapisanych wydatków.</p>";
        return;
    }
    ...
    expenses.forEach(function (expense, index) {
        ...
        amount.textContent = "Kwota: " + expense.amount + " PLN";

        const category = document.createElement("p");
        category.textContent = "Kategoria: " + expense.category;
...
expenseForm.addEventListener("submit", function (event) {
    ...
    if (nameValue === "" || amountValue === "" || categoryValue === "") {
        expenseError.textContent = "Wypełnij wszystkie pola wydatku.";
        return;
    }

    if (Number(amountValue) <= 0) {
        expenseError.textContent = "Kwota musi być większa niż 0.";
        return;
    }
...
contactForm.addEventListener("submit", function (event) {
    ...
    if (isValid) {
        fetch("/api/contact", {
            ...
            .catch(function () {
                successMessage.textContent = "";
                messageError.textContent = "Nie udało się wysłać formularza.";
            });
    }
});
